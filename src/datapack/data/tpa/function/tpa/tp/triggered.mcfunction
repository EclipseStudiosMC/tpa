# Debug:
execute if score .debug_mode tpa.config matches 1 run tellraw @a ["",{"text":"[TPA] ","color":"yellow"},{"text":"Debug: ","color":"blue"},{"text":"Now excecuting: ","color":"gold"},{"text":"tpa:tpa/tp/triggered","color":"dark_aqua"}]
...
# Playsounds:
execute if entity @s[tag=!tpa.not_this_option] at @s run playsound minecraft:entity.player.levelup ambient
